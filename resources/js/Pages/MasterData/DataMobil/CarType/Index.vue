...
// Filter data berdasarkan model dan pencarian
const filteredCarTypes = computed(() => {
    return props.carTypes.filter((item) => {
        const matchModel = selectedModel.value ? item.car_model_id === selectedModel.value : true;
...
const openModal = (mode, item = null) => {
    ...
    if (isEditMode.value && item) {
        ...
        form.car_model_id = item.car_model_id;
        form.name = item.name;
...
const submitForm = () => {
    if (isEditMode.value) {
        form.put(route("car-type.update", selectedId.value), {
            onSuccess: () => closeModal(),
        });
    } else {
        form.post(route("car-type.store"), {
            onSuccess: () => closeModal(),
        });
    }
};
...
const confirmDelete = (id) => {
    if (confirm("Apakah Anda yakin ingin menghapus data ini?")) {
        router.delete(route("car-type.destroy", id));
    }
};
</script>
...
<template>
    <div class="p-4">
        <div class="flex flex-col md:flex-row justify-between items-center mb-4 space-y-4 md:space-y-0">
            <PrimaryButton @click="openModal('create')">Tambah Car Type</PrimaryButton>
            
            <!-- Filter & Search -->
            <div class="flex flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-4 w-full md:w-auto">
                <!-- Filter Model -->
                <select v-model="selectedModel" class="border rounded px-4 py-2 w-full md:w-48">
                    <option value="">Semua Model</option>
                    <option v-for="model in props.carModels" :key="model.id" :value="model.id">
                        {{ model.name }}
                    </option>
                </select>
               ...
               <!-- Kolom Pencarian -->
                 <div class="relative">
                     <TextInput v-model="searchQuery" placeholder="Cari Type Mobil..." class="w-full md:w-48 pl-10 pr-4 py-2 border" />
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full border bg-white rounded-lg shadow-md">
                ...
                <tbody>
                    <tr v-for="(item, index) in filteredCarTypes" :key="item.id" class="border-b hover:bg-gray-100">
                        <td class="px-4 py-2">{{ index + 1 }}</td>
                        <td class="px-4 py-2">{{ item.car_model.name }}</td>
        ...
        <CustomModal :show="modalOpen" :title="isEditMode ? 'Edit Car Type' : 'Tambah Car Type'" @close="closeModal">
            <template #content>
